...
def convolutional_sequence(filter_size, num_filters, image_size, num_channels=3):
    ...
    layers.append(Convolutional(filter_size=filter_size, num_filters=num_filters*4, num_channels=num_channels, use_bias=True, tied_biases=True, weights_init=IsotropicGaussian(0.01),image_size=image_size, name='conv_4'))
    layers.append(LeakyRectifier())
    ...
    layers.append(Convolutional(filter_size=filter_size, num_filters=num_filters*8, num_channels=num_channels, use_bias=True, tied_biases=True, weights_init=IsotropicGaussian(0.01),image_size=image_size, name='conv_5'))
    layers.append(LeakyRectifier())
    layers.append(BatchNormalization(input_dim=layers[14].get_dim('output'), name='batchnorm_5'))

    layers.append(Convolutional(filter_size=filter_size, num_filters=num_filters*8, num_channels=num_channels, use_bias=True, tied_biases=True, weights_init=IsotropicGaussian(0.01),image_size=image_size, name='conv_6'))
    layers.append(LeakyRectifier())
...
def build_model(images, labels):
    ...
    bottom_conv_sequence = convolutional_sequence((3,3), 32, (90, 90))
    ...
    top_mlp = MLP([LeakyRectifier(), Softmax()], [conv_out_dim, 612, 10], weights_init=IsotropicGaussian(), biases_init=Constant(1))
