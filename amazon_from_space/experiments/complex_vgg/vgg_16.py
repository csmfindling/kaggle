...
#basepath = '/Users/csmfindling/Documents/Kaggle/amazon/data/'
import lasagne
...
def build_model(input_var):
    net = {}
    layers_names = {}
    net['mean value'] = shared(numpy.zeros((4,), dtype=config.floatX))
    #X = input_var * numpy.float32(255)
    X = input_var - net['mean value'][None, :, None, None]/numpy.float32(255)

    net['input'] = InputLayer((None, 4, 256, 256), X)
...
def get_model(input_var):
    ...
    net['mean value'].set_value(numpy.concatenate((model_pretrained['mean value'], numpy.zeros(1))).astype(config.floatX))

    count     = 0
    layer_idx = 1
    W_firstlayer = numpy.zeros(net[layers_names[layer_idx]].W.get_value().shape)
    W_firstlayer[:, :3] = w_pretrained[count] * 255.
    W_firstlayer[:, -1] = numpy.random.normal(loc=0, scale=numpy.std(w_pretrained[count] * 255.))
    net[layers_names[layer_idx]].W.set_value(numpy.asarray(W_firstlayer, dtype=numpy.float32))

    for layer_idx in range(2,len(layers_names)):
